...
KinectHand::KinectHand() {
    ...
    refinedMap.allocate(512, 424, OF_IMAGE_GRAYSCALE);
    refinedMap.setColor(ofColor::ghostWhite);
    
    handLocationContainer = ofRectangle(0, 0, 512, 424);
    handLocationContainer.setWidth(ofGetWindowWidth());
    handLocationContainer.setHeight(ofGetWindowHeight());
}
...
void KinectHand::update() {
    ...
    if (kinect.isFrameNew()) {
        ...
        if (counterSumAverage > 20) {
            ...
            //std::cout << "xAvg: " << xAvg << " " << "yAvg: " << yAvg << std::endl;
        }
